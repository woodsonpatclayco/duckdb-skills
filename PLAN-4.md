# PLAN-4 — One local SQL surface over Snowflake extracts and Excel workbooks

> **Version 4**, superseding `PLAN-3.md` (frozen; kept for the record). **One** instruction changed,
> forced by a hard platform limitation found while implementing item 2b. Review the delta only.
>
> **`COPY INTO ... FILE_FORMAT=(TYPE=PARQUET)` cannot unload `TIMESTAMP_TZ` or `TIMESTAMP_LTZ`.**
> Measured: `Error encountered when unloading to PARQUET: TIMESTAMP_TZ and LTZ types are not supported
> for unloading to Parquet.` So **an extract query cannot be a blind `SELECT *`** — see "Transport"
> below for the projection rule and the verified workaround.
>
> Blast radius, measured on `DB_CONTROL_TOWER.SCH_PROJECT_OPERATIONS`: **4 of 197 tables (2.0%)**
> carry such columns, 10 columns in total. A footnote rather than a redesign — but a loud one, and
> `DT_PROJECTS`, one of item 2's two pinned objects, is among the four.
...
- Types survive exactly: `decimal(5,0)`, `decimal(12,2)`, and `'01100'` as VARCHAR with its
  leading zero intact. `DATE` survives as `DATE`.
- **Except `TIMESTAMP_TZ` and `TIMESTAMP_LTZ`, which Parquet unload refuses outright** (version-4
  delta). `COPY INTO` fails with
  `TIMESTAMP_TZ and LTZ types are not supported for unloading to Parquet`. This is not rare by
  accident: `CURRENT_TIMESTAMP()` returns `TIMESTAMP_LTZ(9)`, so any audit-stamped table can carry
  one.
- **The rule: an extract query may not be a blind `SELECT *`.** Before unloading, check
  `INFORMATION_SCHEMA.COLUMNS` for `DATA_TYPE IN ('TIMESTAMP_TZ','TIMESTAMP_LTZ')` and project those
  columns explicitly as `CONVERT_TIMEZONE('UTC', <col>)::TIMESTAMP_NTZ AS <col>`, leaving every other
  column alone. **Verified lossless end to end:** Snowflake's `2019-09-06 05:00:00.000` arrived in
  DuckDB as `2019-09-06 05:00:00` typed `TIMESTAMP`, matching Snowflake's own UTC rendering exactly.
- **The resulting DuckDB type is a naive `TIMESTAMP` holding UTC**, not `TIMESTAMPTZ`. That is the
  same shape as the sidecar's `materialized_at`, so the same UTC-comparison discipline applies —
  compare against `timezone('UTC', now())`, never `now()`.
- The cast is recorded in the sidecar's `query` as provenance, so what landed is always reconstructible
  from what the sidecar says was run.
...
- `COPY INTO` Parquet → `GET` into a local directory it creates first → `REMOVE` the stage copy.
  **The query is TZ-projected, never a blind `SELECT *`** — see the version-4 delta under
  "Transport". A table with no TZ/LTZ column projects to the same thing `SELECT *` would have
  produced, so the rule costs nothing on the 98% and prevents a hard failure on the rest.
